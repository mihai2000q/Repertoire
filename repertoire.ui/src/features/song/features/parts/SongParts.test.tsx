import { emptySong, emptySongPart, reduxRender, withToastify } from '../../../../test-utils.tsx'
...
describe('Song Parts', () => {
  ...
  it.skip('should be able to reorder parts', async () => {
    ...
    server.use(
      http.put('/songs/parts/move-in-song', async (req) => {
  ...
  it('should show the drawer when selecting parts, and the context menu when right-clicking after selection', async () => {
    ...
    await user.pointer({
      keys: '[MouseRight>]',
      target: screen.getByLabelText(`song-part-${parts[0].name}`)
    })
