...
function HeaderPanelCard({
  ...
  return (
    <Box ref={ref} pos={'relative'}>
      ...
      {hideIcons !== true && <Box pos={'absolute'} right={0} top={0} p={0}>
        <Menu position={'bottom-end'} shadow={'md'}>
          <Menu.Target>
            <ActionIcon variant={'grey'} style={{ transition: '0.25s', opacity: hovered ? 1 : 0 }}>
              <IconDots size={18} />
            </ActionIcon>
          </Menu.Target>
      ...
      {hideIcons !== true && <Box pos={'absolute'} right={0} bottom={-12} p={0}>
