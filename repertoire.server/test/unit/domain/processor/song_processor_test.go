...
func TestAddCustomRehearsal_WhenSongHasNoArrangement_ShouldReturnFalse(t *testing.T) {
	// given
	...
	// when
	errCode, updated := _uut.AddCustomRehearsal(mockSong, songSectionRepository)
...
func TestAddCustomRehearsal_WhenSuccessful_ShouldUpdateSongAndSections(t *testing.T) {
	// given
	songSectionRepository := new(repository.SongSectionRepositoryMock)
	progressProcessor := new(ProgressProcessorMock)
	_uut := processor.NewSongProcessor(progressProcessor)
