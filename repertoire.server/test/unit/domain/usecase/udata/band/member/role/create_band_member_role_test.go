...
func TestCreateBandMemberRole_WhenCountBandMemberRolesFails_ShouldReturnInternalServerError(t *testing.T) {
	...
	_uut := role.NewCreateBandMemberRole(userDataRepository, jwtService)
	...
	userDataRepository.On("CountBandMemberRoles", new(int64), userID).Return(internalError).Once()
...
func TestCreateBandMemberRole_WhenCreateBandMemberRoleFails_ShouldReturnInternalServerError(t *testing.T) {
	...
	userDataRepository.On("CountBandMemberRoles", new(int64), userID).Return(nil).Once()

	internalError := errors.New("internal error")
	userDataRepository.On("CreateBandMemberRole", mock.IsType(new(model.BandMemberRole))).
...
func TestCreateBandMemberRole_WhenSuccessful_ShouldNotReturnAnyError(t *testing.T) {
	...
	count := &[]int64{20}[0]
	userDataRepository.On("CountBandMemberRoles", mock.IsType(count), userID).
		Return(nil, count).
		Once()
