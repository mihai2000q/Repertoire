...
func TestValidateCreateSongRequest_WhenSingleFieldIsInvalid_ShouldReturnBadRequest(t *testing.T) {
	tests := []struct {
		...
		{
			"Album Title and ID are invalid because they are not set while the TrackNo is set",
			CreateSongRequest{
				Title:   validSongTitle,
				TrackNo: &[]uint{1}[0],
			},
			[]string{"AlbumID", "AlbumTitle"},
			[]string{"required_with", "required_with"},
		},
...
func TestValidateUpdateSongRequest_WhenSingleFieldIsInvalid_ShouldReturnBadRequest(t *testing.T) {
	tests := []struct {
		...
		// Album ID Test Cases
		{
			"Album ID is invalid because it is null while the TrackNo is set",
			UpdateSongRequest{
				ID:      uuid.New(),
				Title:   validSongTitle,
				TrackNo: &[]uint{1}[0],
